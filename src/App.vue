<template>
  <div id="app" class="container">
    ...
    <!-- 表单 -->
    <div class="info-fo">
      <!-- Form组件有iview提供 ref 相当与dom 里的id
      mvvm 数据第一的 获取数据
       -->
      <Form ref="formValidate" :model="formValidate" :rules="ruleValidate">
        <FormItem label="手机号" prop="phone">
          <i-input placeholder="请输入手机号" size="large" v-model="formValidate.phone">
          </i-input>
        </FormItem>
    ...
    return {
      ...
      ruleValidate: {
        ...
        checkNum: [
          ...
          { type: 'string', min: 4, max: 4, message: '验证码格式错误', trigger: 'blur'}
        ]
      }
    }
  },
  methods: {
    ...
    getcheckNum () {
      if (this.formValidate.phone.length === 11) {
        // this -> vue -> use iview -> Message js 声明调用
        this.$Message.success({
          content: '验证码为: 1234',
          duration: 6,
          closeable: true
